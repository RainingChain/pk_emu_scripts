--[[
Purpose: Help obtaining Pokérus in Pokemon Ruby, Sapphire and Emerald.
...
local gameVersionName = emu:read8(0x80000AE)
 ...
 elseif gameVersionId == 0x50 then
  ...
  elseif gameVersionId == 0x45 then
    ...
    console:log("Error: Only Pokemon Ruby, Sapphire and Emerald are supported.")
    return
 end
...
if gameVersionName == "Ruby" or gameVersionName == "Sapphire" then  
    ...
    if emu:read16(pokerusFuncAddr) == 46448 then
        console:log("Ruby/Sapphire v1.1 or v1.2 detected")
    elseif emu:read16(pokerusFuncAddr) == 512 then
        console:log("Ruby/Sapphire v1.0 detected")
        pokerusFuncAddr = POKERUS_FUNC_ADDRS[2]
    else
        console:log("Error: Unable to determine the Ruby/Sapphire version.")
    end
...
console:log("  - Start a Pokemon battle and attack the wild Pokémon until it faints")
